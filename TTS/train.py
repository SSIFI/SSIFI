...
def main(restore_step):
    ...
    scheduled_optim = ScheduledOptim(optimizer, hp.decoder_hidden, hp.n_warm_up_step, restore_step)
    ...
    try:
        checkpoint = torch.load(os.path.join(checkpoint_path, 'checkpoint_{}.pth.tar'.format(restore_step)))
    ...
    for epoch in range(hp.epochs):
        ...
        for i, batchs in enumerate(loader):
            for j, data_of_batch in enumerate(batchs):
                ...
                current_step = i*hp.batch_size + j + restore_step + epoch*len(loader)*hp.batch_size + 1
...
def start_train(restore_step=0):
